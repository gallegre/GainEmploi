...
graph <- ggplot(basegr %>% filter(name != 'RDISP'), aes(fill = name, y = value, x = typrev)) + 
  ...
  geom_text(
    aes(x = 0.5, y = seuilpm + 150, label = "Seuil de pauvreté monétaire"),
    size = 3, 
...
graph2 <- ggplot(basegr %>% filter(name != 'RDISP'), aes(fill = name, y = value, x = typrev)) + 
  ...
  geom_text(
    aes(x = first(typrev), y = seuilpm + 300, label = "Seuil de pauvreté monétaire"),
    size = 3, 
    hjust = 0, #Left-justified
    colour = "grey"
  ) +
...
graph3 <- ggplot(basegr %>% filter(name != 'RDISP'), aes(fill = name, y = value, x = typrev)) + 
  ...
  geom_text(
    aes(x = 0.5, y = seuilpm + 300, label = "Seuil de pauvreté monétaire"),
    size = 3, 
    hjust = 0, #Left-justified
    colour = "grey"
  ) +
...
```{r}
#| label: fig-TdG
#| fig-cap: Taux de gain pour une reprise d'emploi au Smic à temps-plein, selon la configuration familiale
...
graph4 <- ggplot(basegr, aes(x = casconj, y = TGM_100, colour = enf)) +
  geom_point_interactive(aes(
    tooltip = paste("Taux de gain :", number(TGM_100, accuracy = 1))
  ), size = 3) +  
  theme_minimal() +
