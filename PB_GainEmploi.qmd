...
subtitle: "Gains à l'emploi des bénéficiaires de l'assistance"
...
#| fig-cap: Gain marginaux effectifs selon la configuration familiale et niveau de revenu
...
graphG<-ggplot(base, aes(x = activite, y = gainmarginal, colour = cas)) +
  geom_point_interactive(aes(
    tooltip = paste("Gain marginal :", gainmarginal, accuracy = 0)
  ), size = 3) +  
  geom_line(aes(group = cas), size = 1) +
  theme_minimal() +
...
#afaire : augmenter l'espace entre les groupes d'histogrammes, trouver des couleurs...
```
...
## 1.3 Que change la prise en compte des coûts de garde des jeunes enfants ?

A commenter...

```{r fig_garde}
#| label: fig-garde
#| fig-cap: Revenu disponible après coût de garde selon revenu et configuration familiale
