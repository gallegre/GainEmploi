...
citation:
  ...
  issue: 2025-25p
  url: https://xxx.github.io/GainEmploi/
annee: 2025
wp: 25p
date: "2025-11-22"
date-modified: today
...
format:
  ...
  wp-pdf:
    toc: false
    output-file: "OFCEWP2025-25p.pdf"
editor: visual
...
sofi <- readxl::read_xlsx("Maquette Sofi 2024 -08-11-2024 - cas types smic -mp.xlsx", sheet = "synthèse")
...
labels_rev <- c("Salaire" = "Salaire", "PF" = "Prestations familiales", "PL" = "#238A8DFF", 
...
graph <- ggplot(basegr %>% filter(name !='RDISP'), aes(fill = name,
  ...
  scale_fill_manual(
   ...
   geom_bar_interactive(
    ...
    guides(
    fill = guide_legend(title = NULL, order = 1),  # La légende des couleurs en premier
    colour=guide_legend(title = NULL, order = 2)  # La légende du point en dernier
  ) +
  ...
  #   axis.text.x = element_text(size = rel(0.7), angle = 40, hjust = 1,vjust = 1)
  # ) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 2500), breaks = seq(0, 2500, by = 500)) + 
  theme(panel.border = element_blank())+
  # labs(caption="Source : législation avril 2024, maquette Sofi https://github.com/murielpucci/SOFI,\ncalculs des auteurs")+
  # theme(
  #   plot.caption = element_text(size = rel(0.5)) 
  # ) +
  ofce_caption(
    champ = "Familles sans enfant",
    lecture = "Les couleurs sont jolies",
    note = "Les calculs sont probablement à revoir",
    source = "législation avril 2024, {.orange maquette Sofi} https://github.com/murielpucci/SOFI")
...
Le niveau de vie d'un célibataire sans enfant sans revenus d'activité et recourant à toutes les prestations est de 851 euros par UC, ce qui est inférieur de 35% au seuil de pauvreté monétaire : son intensité de pauvreté (c'est-à-dire l'écart au seuil de pauvreté) est donc de 35%. Un emploi à mi-temps au Smic ne lmonétaire ui permet pas de franchir le seuil de pauvreté (intensité de 9%). Le célibataire sans enfant franchit le seuil de pauvreté monétaire avec un Smic à plein temps même s'il ne recourt pas à la prime d'activité et son niveau de vie atteint 126% du seuil s'il y recourt. En revanche, un emploi à mi-temps ne permet pas de sortir de la pauvreté même si la prime d'activité en réduit l'intensité (de 30% avant prime à 9% après).
...
```{r fig-minimasociaux, eval=FALSE}
...
graph4 <- Bareme_long %>%
  ...
  labs(caption="base 100 = 1990 ; Smic horaire et minima sociaux en euros constants ;
  Source : DREES ; calculs des auteurs")+
  labs(x="", y="")+
  theme(
