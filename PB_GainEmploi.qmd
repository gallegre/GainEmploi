...
![Revenu disponible d'une personne seule selon son revenu professionnel](){#fig-1}
...
![Revenu disponible d'un couple avec deux enfants](){#fig-2}
...
graph2 <- ggplot(basegr %>% filter(name != 'RDISP'), aes(fill = name, y = value, x = typrev)) + 
  ...
  geom_text(
    aes(x = first(typrev), y = seuilpm + 50*2.1, label = "Seuil de pauvreté monétaire"),
    size = 3, 
    hjust = 0, #Left-justified
    colour = "grey"
  ) +
...
ggsave(filename="graph/Evol.png", plot=graph3)

htmltools::save_html(igraph3, "graph/Evolh.html")
