...
graph <- ggplot(basegr %>% filter(name !='RDISP'), aes(fill = name,
  ...
  labels = c("PA" = "Prime d'activité",
   ...
   geom_bar_interactive(
    ...
    colour=guide_legend(order = 2)  # La légende du point en dernier
  ) +
  theme(
  ...
  scale_fill_manual(
   ...
   geom_bar_interactive(
    ...
    aes(
      ...
      tooltip=paste("Revenu disponible net \n du coût de la garde :",round(value,0), "euros")), 
    size = 3
  ) +
  
labs(title = "Revenu disponible d'une famille avec un enfant âgé de 1 \nan gardé par sa mère et/ou une assitante maternelle")+
    guides(
    fill = guide_legend(order = 1),  # La légende des couleurs en premier
    colour=guide_legend(order = 2)
  ) +
  ...
  scale_y_continuous(limits = c(-500, 4000), breaks = seq(-500, 4000, by = 500)) + 
  theme(panel.border = element_blank())


igraph_cmg <- girafe(ggobj = graph_cmg)
...
#Guillaume : au choix un graphique avec les 4 ou deux graphiques comme actuellement
castyp <- read_excel("cas types smic.xlsx",sheet = "synthese")
...
graph4 <- ggplot() +
  ...
  labs(title = "Avec ou sans prime d'activité") +
  theme_minimal() +
  ...
  theme(
    ...
    axis.line = element_line(color = "black", linewidth = 0.2, linetype = "solid"),
    axis.text.x = element_text(size = rel(0.8), angle = 40, hjust = 1, vjust = 1),
    axis.text.y = element_text(size = rel(0.8), angle = 40, hjust = 1, vjust = 1),
    strip.text= element_text(size = rel(0.7),face="bold"),
    plot.title = element_text(size = rel(0.9), color="#5E4FA2",face="bold")
  )+
...
graph4a <- ggplot() +
  geom_point_interactive(
    data = basegr, aes(x =casconj, y = TGM, 
      colour = enf, 
      tooltip=paste("Taux effectif de gain à l'emploi :",TGM, "%")), 
    size = 2
  )+
...
graph4b <- ggplot() +
  geom_point_interactive(
    data = basegr, aes(x =casconj, y = TGMspa, 
      colour = enf, 
      tooltip=paste("Taux effectif de gain à l'emploi sans prime d'activité :",TGMspa, "%")), 
    size = 2
  )+
  facet_wrap(~emploi,nrow=1)+
  theme_minimal() +
...
graphG2<-ggplot() +
 ...
 labs(
   # title="Avec ou sans prime d'activité",
 x = "Revenu individuel en parts de smic",
  ...
  theme(
    ...
    panel.border = element_rect(color = "black", fill = NA, linewidth = 0.3),
    legend.key.size = unit(0.5, "lines"),  # taille des carrés
    plot.title = element_text(size = 10, color = "#5E4FA2", face = "bold")
    )



igraphG2<-girafe(ggobj = graphG2)
...
graph_pm<-ggplot() +
    geom_point_interactive(
    data = base_long, aes(x =nbenf2, y = round(montant,0), 
      colour = emp, 
  ...
  theme(
    ...
    axis.title.y = element_blank(),
    legend.title = element_blank(),
    plot.title = element_text(size = rel(1.5),color="#5E4FA2",face="bold"),
    legend.position = "bottom",
    legend.box = "vertical",
...
igraph_pm<-girafe(ggobj = graph_pm)
...
srcv<-read_excel("Sorties_Privation.xlsx",sheet = "Sorties_Privation")
...
graph5 <- ggplot(base, aes(x = isocou, y = NVIE, fill = enf)) + 
  ...
  scale_linetype_manual(name = "Références", values = c("Seuil de pauvreté monétaire (60 %)" = "solid", "Seuil de grande pauvreté monétaire (40 %)" = "dashed"),
    ...
    guides(
    fill = guide_legend(order = 1),  # La légende des couleurs en premier
    linetype = guide_legend(order = 2,nrow=2) # La légende des lignes en dernier,
  ) +
  theme(
    axis.title.x = element_blank(),
    axis.title.y = element_blank(),
    ...
    legend.position="bottom",
    legend.box = "vertical",
    legend.text=element_text(rel=0.6),
    legend.key.size = unit(0.3, units="cm"),
    legend.spacing.y = unit(0.001, "cm"),
    plot.margin = margin(t = 20, r = 10, b = 20, l = 10),
    axis.line = element_line(color = "black", linewidth = 0.2, linetype = "solid"),
    axis.text.x = element_text(size = rel(0.9), angle = 00, hjust = 1, vjust = 1)
  )
